...
class M_Kelas extends CI_Model {
    ...
    function update(){
        ...
        $data1 = array('kompetensi_keahlian' => $this->input->post('kompetensi_keahlian'),
                        'nama_kelas' => $this->input->post('nama_kelas'));

        $data2 = array('nama_kelas' => $this->input->post('nama_kelas'));

        if(empty($this->input->post('kompetensi_keahlian'))) {
            $this->db->where($where);
